...
commands=("composer update --lock" "composer test" "composer format" "composer trim:packages" "composer normalize" "badgen -j version -s $VERSION -c green > ./badges/version.svg" "git add . && git commit -m 'update: $VERSION' && git push" "git tag $VERSION && git push origin $VERSION" "update_version $PATCHTYPE")
